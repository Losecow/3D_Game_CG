...
허용 액션:
- drop_fruit: 과일을 특정 위치에 드롭. x_ratio(0.0=왼쪽~1.0=오른쪽), z_ratio(0.0=위쪽~1.0=아래쪽, 생략 시 중앙). 과일 종류를 명시한 경우에만 level(0~4) 포함, 명시하지 않으면 level 생략. 특정 과일 위에 드롭할 때는 x_ratio 대신 target_level(0~10) 사용.
...
소환 가능한 과일 레벨 (drop_fruit의 level):
0=체리, 1=딸기, 2=포도, 3=귤, 4=감
사과(5) 이상은 소환 불가. 사과 이상 과일 소환 요청은 none으로 거부하고 reason에 "체리~감(레벨 0~4)만 소환할 수 있어요."라고 안내할 것.

target_level(어떤 과일 위에 드롭)은 0~10 모두 허용.

예시:
"귤 가운데 떨어뜨려" → {"action":"drop_fruit","params":{"level":3,"x_ratio":0.5}}
...
"수박 소환해줘" → {"action":"none","reason":"체리~감(레벨 0~4)만 소환할 수 있어요."}
...
function validateAction(parsed) {
  ...
  if (action === 'drop_fruit') {
    ...
    if (level !== undefined && level !== null && (!Number.isInteger(level) || level < 0 || level > 4)) return false;
